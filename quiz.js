...
// Проверяем, существует ли элемент quizContainer перед инициализацией
...
function showQuestion(step) {
  ...
  if (!quizContainer) return; // Выходим, если элемент не найден
...
function saveAnswer() {
  // Проверяем, существует ли элемент quizContainer
  if (!document.getElementById('quizContainer')) return false;
...
if (nextBtn) {
  nextBtn.addEventListener('click', () => {
    ...
    try {
      ...
      // Добавляем обработчик для кнопки "Подробнее" только если элементы существуют
